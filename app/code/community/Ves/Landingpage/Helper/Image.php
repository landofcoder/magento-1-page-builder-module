 ...
 * @author Magento
 */
class Ves_ContentTab_Helper_Image extends Mage_Core_Helper_Abstract
{
    /**
     ...
     * @var string
     */
    const MEDIA_PATH    = 'ves_contenttab';
    ...
    {
        return Mage::getBaseDir('media') . DS . self::MEDIA_PATH . '/' . $path;
    }
    ...
    {
        return Mage::getBaseUrl('media') . '/' . self::MEDIA_PATH . '/' . $path;
    }
     ...
     * @return bool|string
     */
    public function uploadImage($scope,$data,$path)
    {
        ...
        if ($adapter->isUploaded($scope)) {
            ...
            if (!$adapter->isValid($scope)) {
                Mage::throwException(Mage::helper('ves_contenttab')->__('Uploaded image is not valid'));
            }*/
            ...
            if ($upload->save($this->getBaseDir($path),$data)) {
                return self::MEDIA_PATH . '/' . $path."/".$data;
            }
     ...
     * @return bool|string
     */
    public function resize(ves_contenttab_Model_Location $item, $width, $height = null)
    {
        if (!$item->getImage()) {
            return false;
        }
